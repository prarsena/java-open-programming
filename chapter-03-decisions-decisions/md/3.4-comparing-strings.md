...
if (str1.equals(str2)) { // This is true
    IO.println("str1 and str2 are equal.");
}

if (str1.equals(str3)) { // This is false
    IO.println("str1 and str3 are equal.");
} else {
    IO.println("str1 and str3 are NOT equal.");
}
```
...
if (username1.equalsIgnoreCase(username2)) { // This is true
    IO.println("Usernames are the same, ignoring case.");
}
```
...
1. Given `String pass1 = "secret";` and using `IO.readln` to read user input into a `String pass2`, write an `if-else` statement that correctly checks if the user's input matches `pass1` and prints "Access granted" or "Access denied" accordingly.
