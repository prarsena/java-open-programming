...
Variables in Java _should_ always have a type. However, Java 10 introduced the `var` keyword, which allows the compiler to infer the data type based on the initial value. This is widely used in modern Java to reduce verbosity, especially when the type is obvious from the context.

```Java
// Using var (Common in modern Java)
var str = "Hello";  // Compiler infers String
var num = 123;      // Compiler infers int
var dbl = 45.67;    // Compiler infers double
```

While `var` is convenient, remember that Java is still a **strongly typed language**. Once `var` infers a type (e.g., `int`), you cannot assign a different type (e.g., `String`) to that variable later.
