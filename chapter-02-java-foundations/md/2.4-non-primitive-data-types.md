...
In classic Java, creating a string that spans multiple lines required using newline characters (`\n`) and concatenation (`+`). Modern Java (Java 15+) introduces **Text Blocks**, which use triple quotes (`"""`) to preserve the format of the text exactly as you type it.
...
IO.println(s.charAt(0)); // returns 'S'
IO.println(s.charAt(1)); // returns 'A'

IO.println(s.equalsIgnoreCase("SARAH")); // returns true
IO.println(s.equalsIgnoreCase("SAMANTHA")); // returns false
```

## Simple Input with `IO.readln`
For simple text input, modern Java provides the `IO.readln` method. It prints a prompt and returns the user's input as a `String`.
...
This is much simpler than using `Scanner` for basic text. However, for reading numbers or more complex input, `Scanner` is still the go-to tool.
...
To use `Scanner`:
1. **Import**: In classic Java, you must import the `Scanner` class at the top of your Java file. In modern Java (implicit classes), `java.util.Scanner` is imported automatically.
   
```Java
import java.util.Scanner; // Required in classic Java
```
...
```Java
void main() {
    // Scanner is automatically imported in modern Java
    Scanner keyboard = new Scanner(System.in); 
    ...
    IO.print("Enter your GPA: ");
    double gpa = keyboard.nextDouble(); // Read a double

    IO.println("Hello, " + name + "! You are " + age + " years old with a GPA of " + gpa);

    keyboard.close(); // It's good practice to close the scanner when done
}
```

### Classic Java Scanner Example
In older Java versions, you need the full class structure and explicit imports:
...
void main() {
    ...
    IO.print("Enter a line of text: ");
    String text = keyboard.nextLine(); // Now reads the actual text
    
    IO.println("You entered: " + number + " and \"" + text + "\"");
...
IO.println(strNum + " apples"); // Output: 100 apples
```
...
IO.println("Square root of 64: " + squareRoot);

double powerResult = Math.pow(2, 8); // powerResult will be 256.0
IO.println("2 to the power of 8: " + powerResult);
```

For a comprehensive list of `Math` methods, refer to the Java documentation or resources like W3Schools.
...
4. What is the purpose of `import java.util.Scanner;`?
5. If you use `keyboard.nextInt()` to read a number, what common issue might arise if `keyboard.nextLine()` is called immediately afterwards, and how do you fix it?
6. What happens if a user types "abc" when a `Scanner` is expecting `nextInt()`?
7. What is the purpose of the `final` keyword?
8. What is the naming convention for Java constants?
9. What is the purpose of a wrapper class for a primitive data type?
10. How do the names of wrapper classes typically differ from their corresponding primitive types?
11. If you have the `String` "45.7", which parse method would you use to convert it to a `double`?
12. Do you need to create an object of the `Math` class to use its methods? Why or why not?
13. Which `Math` method would you use to calculate 5 cubed (5 to the power of 3)?
