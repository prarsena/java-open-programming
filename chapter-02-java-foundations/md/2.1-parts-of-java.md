...
Every Java program follows a basic structure. In modern Java (Java 25+), this structure is very simple.
...
- **`void main()`**: This is the main method, the entry point of every Java application.  
    - `void`: This indicates that the method does not return any value.
    - `main`: This is a special name that the Java Virtual Machine (JVM) looks for when starting a program.
      
- **`IO.println("Hello, World!");`**: This is the line that actually performs an action – it prints "Hello, World!" to the console. We'll discuss this more in the next section.
  
- **`{ }`**: Curly braces define code blocks. They indicate the start and end of methods and other control structures.

### Classic Java Structure
Before Java 25, the structure was more verbose. You will still see this in older codebases, tutorials, or when building large applications where explicit class structures are necessary.
...
You may be able to understand some of the code, while other aspects may be unclear. Don't worry about memorizing every part of it, but here's an explanation of each part:

- **`public class HelloWorld`**: This line declares a class named `HelloWorld`. In classic Java, all code resides within explicitly named classes.
- **`public static void main(String[] args)`**: The classic main method signature.
    - `public`: Accessible from anywhere.
    - `static`: This method belongs to the `HelloWorld` class itself, not to a specific instance of the class.
    - `String[] args`: This declares a parameter named `args`, which is an array of `String` objects for command-line arguments.
- **`System.out.println`**: The fully qualified way to call the print method. Modern Java imports this automatically for you in simple programs.
...
Java provides simple ways to display output to the console. In modern Java programs (using implicit classes), you can use `IO.println` and `IO.print` directly. In classic Java, you use `System.out.println` and `System.out.print`.

**`IO.println()`** (or `System.out.println()`): This method prints the specified content to the console and then moves the cursor to the next line.
  
```java
IO.println("This is the first line.");
IO.println("This is the second line.");
```
...
**`IO.print()`** (or `System.out.print()`): This method prints the specified content to the console but does _not_ move the cursor to the next line. Subsequent output will appear on the same line.

```Java
IO.print("This is on the same line. ");
IO.print("Still on the same line.");
IO.println(" Now a new line.");
```
...
```java
IO.println("He said, \"Hello, World!\"");
```
...
IO.println("Hello"); // This also works
```
...
The `+` operator in `IO.print` and `IO.println` can be used to concatenate (join) strings and other data types for output.
...
2. How does the modern `main` method differ from the classic `public static void main(String[] args)`?
3. What is the key difference between `IO.print()` and `IO.println()`?
4. Write a single `IO.print()` statement that would produce the output: `HelloThere`.
5. Why are comments important in programming?
6. Which type of comment would you use for a brief explanation of a single line of code?
7. What is the difference between declaring a variable and initializing it?
8. How would you combine the `String` literal "My name is " with a `String` variable `userName` and an `int` variable `userAge` to print a complete sentence?
...
1. **Greeting Program**: Write a Java program that declares two `String` variables, `firstName` and `lastName`, and initializes them with your first and last name. Then, print a greeting message to the console that says "Hello, [firstName] [lastName]!" using `IO.println()` and the `+` operator.
