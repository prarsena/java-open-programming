...
IO.println(a - b); // 7
IO.println(a * b); // 30
IO.println(a / b); // 3 (Integer division rounds down!)
IO.println(a % b); // 1 (10 divided by 3 is 3 with a remainder of 1)
```
...
// 2. 6 / 2 = 3
// 3. 3 * 3 = 9
IO.println(calculation); // Output: 9
```
...
IO.println(result); // Output: 100.0
```
...
IO.println(decCast); // Output: 100 (the decimal part is truncated)
```
...
IO.println(mixedResult); // Output: 12.5
```
...
4. Rewrite `total = total - amount;` using a combined assignment operator.
5. If `value` is `7`, what is `value` after `value %= 3;` ?
6. Explain the difference between widening and narrowing conversion.
7. What happens if you cast a `double` value of `15.75` to an `int`?
8. Why is an explicit cast required when converting a `long` to an `int`?
