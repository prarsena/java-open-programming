...
## PrintWriter: Writing Data to a File
...
> ‼️ ⚠️ 👺 **Warning:** If the file already exists, creating a new `PrintWriter` instance by passing only the filename argument will **OVERWRITE** the existing file.
...
void main() throws FileNotFoundException {  
    ...
    for (i=1;i<4;i++){  
        String studentName = IO.readln("Enter student " + i + "'s name: ");  
        outputFile.println(studentName);  
    }  
    IO.println("Wrote " + (i-1) + " student names to file.");  
    outputFile.close();  
}
```
...
void main() {  
        try {  
            ...
            IO.println("Can't write to this location \n" + e );  
        }  
}
...
## FileWriter: Appending Data to a File
...
void main() throws IOException {
    ...
    while (addMoreUsers){
        ...
        IO.println(String.format("Added %s. %n", username));
        
        String response = IO.readln("Add another user? (Y/n) ");
